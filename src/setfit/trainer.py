...
class ColumnMappingMixin:
    ...
    def _validate_column_mapping(self, dataset: "Dataset") -> None:
        ...
        if self.column_mapping is None and not self._REQUIRED_COLUMNS.issubset(
            column_names
        ):
        ...
        if self.column_mapping is not None:
            ...
            missing_columns -= set(dataset.column_names) - set(
                self.column_mapping.keys()
            )
            if missing_columns:
                raise ValueError(
...
class Trainer(ColumnMappingMixin):
    ...
    def __init__(
        ...
        _framefit: bool = False,
    ) -> None:
        self._framefit = _framefit
        if self._framefit:
            self._REQUIRED_COLUMNS = {"image", "label"}
        else:
            self._REQUIRED_COLUMNS = {"text", "label"}

        if args is not None and not isinstance(args, TrainingArguments):
            raise ValueError(
                "`args` must be a `TrainingArguments` instance imported from `setfit`."
            )
        ...
        if train_dataset:
            ...
            if self.column_mapping is not None:
                ...
                train_dataset = self._apply_column_mapping(
                    train_dataset, self.column_mapping
                )
        ...
        if eval_dataset:
            ...
            if self.column_mapping is not None:
                ...
                eval_dataset = self._apply_column_mapping(
                    eval_dataset, self.column_mapping
                )
        ...
        # Setup the callbacks
        default_callbacks: List[DefaultFlowCallback] = (
        ...
        if _framefit:
            tokenizer = self.model.model_body[0].processor.tokenizer
        else:
            tokenizer = self.model.model_body.tokenizer

        self.callback_handler = CallbackHandler(
            callbacks, self.model, tokenizer, None, None
        )
        self.state = TrainerState()
        self.control = TrainerControl()
        self.add_callback(
            DEFAULT_PROGRESS_CALLBACK
            if self.args.show_progress_bar
            else PrinterCallback
        )
    ...
    def apply_hyperparameters(
        self, params: Dict[str, Any], final_model: bool = False
    ) -> None:
    ...
    def train(
        ...
        **kwargs,
        ...
        full_parameters = (
            train_parameters + self.dataset_to_parameters(self.eval_dataset)
            if self.eval_dataset
            else train_parameters
        )
    ...
    def train_embeddings(
        ...
        train_max_pairs = (
            -1 if args.max_steps == -1 else args.max_steps * args.embedding_batch_size
        )
        ...
        if x_eval is not None and args.evaluation_strategy != IntervalStrategy.NO:
            eval_max_pairs = (
                ...
                else args.eval_max_steps * args.embedding_batch_size
            )
            eval_dataloader, _, _, _ = self.get_dataloader(
                x_eval, y_eval, args=args, max_pairs=eval_max_pairs
            )
        else:
    ...
    def get_dataloader(
        ...
        input_data = [
            InputExample(texts=[text], label=label) for text, label in zip(x, y)
        ]
        ...
        if args.loss in [
            ...
            dataloader = DataLoader(
                data_sampler, batch_size=batch_size, drop_last=False
            )
    ...
    def _train_sentence_transformer(
        ...
        optimizer = torch.optim.AdamW(
            optimizer_grouped_parameters, **{"lr": args.body_embedding_learning_rate}
        )
        scheduler_obj = model_body._get_scheduler(
            optimizer,
            scheduler="WarmupLinear",
            warmup_steps=warmup_steps,
            t_total=self.state.max_steps,
        )
        ...
        for epoch in range(args.embedding_num_epochs):
            ...
            for step in range(steps_per_epoch):
                ...
                features = list(
                    map(
                        lambda batch: batch_to_device(batch, self.model.device),
                        features,
                    )
                )
                ...
                if args.use_amp:
                    ...
                    torch.nn.utils.clip_grad_norm_(
                        loss_func.parameters(), max_grad_norm
                    )
            ...
            self.maybe_log_eval_save(
                model_body, eval_dataloader, args, scheduler_obj, loss_func, loss_value
            )
        ...
        if self.args.load_best_model_at_end and self.state.best_model_checkpoint:
            ...
            if dir_name.startswith("step_"):
                ...
                logger.info(
                    f"Loading best SentenceTransformer model from step {step_to_load}."
                )
        ...
        num_train_samples = (
            self.state.max_steps * args.embedding_batch_size
    ...
    def maybe_log_eval_save(
        ...
        if self.control.should_log:
            ...
            metrics = {
                "embedding_loss": round(loss_value.item(), 4),
        ...
        if self.control.should_evaluate and eval_dataloader is not None:
            eval_loss = self._evaluate_with_loss(
                model_body, eval_dataloader, args, loss_func
            )
            learning_rate = scheduler_obj.get_last_lr()[0]
            metrics = {
                "eval_embedding_loss": round(eval_loss, 4),
                "learning_rate": learning_rate,
            }
            self.control = self.log(args, metrics)
    ...
    def _evaluate_with_loss(
        ...
        losses = []
        eval_steps = (
            min(len(eval_dataloader), args.eval_max_steps)
            if args.eval_max_steps != -1
        ...
        for step, data in enumerate(
            ...
            labels = labels.to(self.model.device)
            features = list(
                map(lambda batch: batch_to_device(batch, self.model.device), features)
            )
    ...
    def _checkpoint(
        self, checkpoint_path: str, checkpoint_save_total_limit: int, step: int
    ) -> None:
        ...
        if checkpoint_save_total_limit is not None and checkpoint_save_total_limit > 0:
            ...
            for subdir in Path(checkpoint_path).glob("step_*"):
                if subdir.name[5:].isdigit() and (
                    ...
                    or subdir != Path(self.state.best_model_checkpoint)
                ):
                    old_checkpoints.append(
                        {"step": int(subdir.name[5:]), "path": str(subdir)}
                    )
    ...
    def evaluate(
        self, dataset: Optional[Dataset] = None, metric_key_prefix: str = "test"
    ) -> Dict[str, float]:
        """
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        if backend is None:
            ...
            if backend is None:
                raise RuntimeError(
                    "optuna should be installed. To install optuna run `pip install optuna`."
                )
